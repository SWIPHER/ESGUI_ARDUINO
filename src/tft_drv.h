...
/* =================== ② 逻辑分辨率 + 1:1 映射 + 安全区居中 ===================
 * 这块屏是"圆角面板"：四角有圆弧，贴边的文字会被圆角切掉；同时希望字大一些、清晰一些。
 *
 * 本工程的做法（两步，配合 platformio.ini 里的字体接线）：
 *   ① 字库换成**按最终尺寸渲染**的真·30px 字库（src/font_big.c，由 tools/gen_font_big.py 生成）
 *      —— 不是把 16px 点阵像素放大（那样只会把笔画拉成 2x2 方块，越大越糊）；
 *   ② TFT_ZOOM = 1（1 个逻辑像素 = 1 个面板像素，**整数映射、零重采样**），
 *      逻辑区 216x272 在 240x284 面板上居中 → 左右各留 12 列、上下各留 6 行安全边距，
 *      圆角永远切不到内容，文字也保持 1:1 锐利。
 *
 * 为什么不放大：像素放大（TFT_ZOOM=2）不会产生新细节——16px 字库里复杂汉字只有 1px 笔画，
 * 放大后所有笔画都变 2px、笔画间空隙被挤掉，于是"变大了但更糊"。真要更大的字，
 * 就用 tools/gen_font_big.py 生成更大的字库（--size 34 等），并保持 TFT_ZOOM=1。
 *
 * 想调字号/边距：
 *   · 字号：python3 tools/gen_font_big.py --size 26/30/34 …（会同时打印"每行几个汉字/行距"）
 *   · 边距：改下面的逻辑尺寸（保持 8 的倍数），TFT_OFFSET_X/Y 会自动重新居中
 */
#define TFT_ZOOM       1            /* 1 = 1:1（配真字库最清晰）；2/3 仅用于"没有大字号字库"时应急放大 */

#define ESGUI_LOGIC_W  216          /* ← 逻辑宽（必须 8 的倍数）*/
#define ESGUI_LOGIC_H  272          /* ← 逻辑高（必须 8 的倍数）*/
