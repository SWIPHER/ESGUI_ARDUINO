...
/* ==================== 手感参数（想调手感就改这里） ====================
 * ★ 注意：这些是**屏幕像素**阈值（触摸坐标就是面板像素），
 *   本工程一个菜单行高 = 31(字行高) + 6(条目间距) = 37px，所以"一格"取 36 左右 ≈ 一行。
 *   改字库字号后记得同步调这里（行高变了，一格的距离也变了），见 change_font.md。 */
#define TOUCH_EVT_QUEUE     8       /* 一次滑动最多排多少个事件（= 一次最多翻几格） */
#define TOUCH_STEP_PX       36      /* 滑过多少像素算"一格"（≈ 一条菜单的高度 37px） */
#define TOUCH_SWIPE_MIN_PX  34      /* 位移超过它才判定为"滑动"；小于它就是"轻点" */
