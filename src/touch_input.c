 ...
 *   本工程一个菜单行高 = 33(字行高) + 6(条目间距) = 39px，所以"一格"取 38 左右 ≈ 一行。
 *   改字库字号后记得同步调这里（行高变了，一格的距离也变了）。 */
#define TOUCH_EVT_QUEUE     8       /* 一次滑动最多排多少个事件（= 一次最多翻几格） */
#define TOUCH_STEP_PX       38      /* 滑过多少像素算"一格"（≈ 一条菜单的高度 39px） */
#define TOUCH_SWIPE_MIN_PX  36      /* 位移超过它才判定为"滑动"；小于它就是"轻点" */
