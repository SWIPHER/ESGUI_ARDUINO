...
/* ★ 关键：先判"与裁剪区完全不相交"→ 直接丢弃，绝不能走到下面的钳位。
 *   否则越界的单轴图元（eui_draw_hline/vline 传进来的 y1==y2 / x1==x2）
 *   会被"钳"进裁剪区，凭空多画一条幽灵线：
 *     · y 在裁剪区上方 → 被钳到裁剪区首行（分块刷新时 = 每条带第 0 行）
 *     · y 在裁剪区下方 → 被钳到裁剪区末行（= 每条带最后一行）
 *   这正是"条带刷新时每条带首/末行多出一条满宽白线"的根因。 */ \
if ((_x2) < _clip->x1 || (_x1) > _clip->x2 || \
    (_y2) < _clip->y1 || (_y1) > _clip->y2) return; \
