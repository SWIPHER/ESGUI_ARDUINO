...
void ESGUI_AnimTick_CB(ESGUI_T *ui, eui_uint32_t tick) {
    ...
    if(anim_is_running()) {
        ...
        /* ★ 动画刚停下的这一 Tick 必须补一帧重绘（重要）：
         * 上一 Tick 画出的"最后一帧"里还叠加着页面切换的百叶窗遮罩
         * （canvas_apply_transition_mask 的 0xAA 交错扫描线，trans_count 通常停在 1
         *   → 0x55 → 屏幕留下满屏横线），而动画的 ready_cb 只把 trans_active 清零、
         * 并不请求重绘；ESGUI_Tick 的重绘条件是 need_refresh || anim_running，
         * 此时两者都为 0 → 这帧残留会一直挂在屏上，直到用户下次点击才被清掉。
         * 这里借用"上一 Tick 在跑、本 Tick 已停"的下降沿，强制刷一帧干净画面。 */
