 ...
 * @note  本工程 240x280 触摸屏取 6：字体行高 15 + 6 = 条目步进 21px，280 高约 12~13 条/屏；
 *        若纯触摸操作、希望更难点错行，可调到 8~10（代价是每屏可见条数减少）。
 */
#define ESGUI_ITEM_SPACING       6
#endif
 ...
 * @note  值越大，页面淡入/淡出越慢
 */
#define ESGUI_PAGE_TRANSITION_ANIM_TIME 350
#endif
 ...
 * @note  本工程 240x280 取 24。键盘总高 = 行数 × 本值（字母页 4 行 = 96px，数字页 5 行 = 120px），
 *        键盘弹窗高度需 ≥ 键盘总高 + 输入框高(字体行高 + 4 = 19px)，否则键盘会垂直滚动（可用但不顺手）。
 *        → 键盘弹窗建议 232x140：字母页 4 行全显，数字页 5 行(120px) + 输入框也刚好全显。
 */
#define ESGUI_KEY_BOARD_KEY_H  24
#endif
 ...
 * @note  本工程 240x280 取 64：WPA2 密码最长 63 字符 + '\0' 正好够用。
 */
#define ESGUI_KEYBOARD_EDIT_MAX_LEN  64
#endif
